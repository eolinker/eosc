...
type IService interface {
	...
	CommitHandler(namespace string, data []byte) (err error)

	// ProcessHandler 节点propose信息前的处理
	ProcessHandler(namespace string, command string, body interface{}) (data []byte, err error)
...
type IRaft interface {
	Send(namespace, cmd string, body map[string]interface{}) error
	Encode(namespace, cmd string, body map[string]interface{}) ([]byte, error)
	Decode([]byte) (namespace, cmd string, body map[string]interface{}, err error)
}
