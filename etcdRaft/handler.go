...
func (e *EtcdServer) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	e.mu.RLock()
...
func (e *EtcdServer) genHandler() http.Handler {
	sm := http.NewServeMux()
	// 其他节点加入集群的处理
	//sm.HandleFunc("/raft/node/join", nil)
	//
	//sm.HandleFunc("/raft/node/join/try", nil)
	//
	//sm.HandleFunc("/raft/node/join/callback", nil)

	sm.HandleFunc("/raft/node/join", e.join)

	sm.Handle("/", NewPeerHandler(e.server))
	return sm
}


func (e *EtcdServer) join(w http.ResponseWriter, r *http.Request) {
	...
	response.Members, response.Name, err = e.addMember(request.Addr)
	if err != nil {
		panic(err)
	}
...
//joinCallback 分配节点信息
//func (e *EtcdServer) joinCallback(w http.ResponseWriter, r *http.Request) {
...
//func (e *EtcdServer) joinTryPost(w http.ResponseWriter, r *http.Request) {
//	if !rc.join {
...
//
////joinTryGet 获取校验码
//func (e *EtcdServer) joinTryGet(w http.ResponseWriter, r *http.Request) {
//	ip := r.URL.Query().Get("broadcast_ip")
//	port := r.URL.Query().Get("broadcast_port")
//	protocol := r.URL.Query().Get("protocol")
//	address := r.URL.Query().Get("address")
...
//}
//
//func (e *EtcdServer) joinTry(w http.ResponseWriter, r *http.Request) {
//	switch r.Method {
//	case http.MethodGet:
//		e.joinTryGet(w, r)
...
//func (e *EtcdServer) joinHandler(w http.ResponseWriter, r *http.Request) {
