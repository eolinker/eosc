...
func (e *EtcdServer) genHandler() http.Handler {
	sm := http.NewServeMux()
	// 其他节点加入集群的处理
	sm.HandleFunc("/raft/node/join", e.joinHandler)

	sm.HandleFunc("/raft/node/join/try", e.joinTry)

	sm.HandleFunc("/raft/node/join/callback", e.joinCallback)

	sm.Handle("/", e.server.RaftHandler())
	return sm
}
